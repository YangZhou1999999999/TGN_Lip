...
parser.add_argument('--modify_regularization', type=float, default=0.000, help='coefficient to modify the regularization term')
parser.add_argument('--backprop_every', type=int, default=1, help='Every how many batches to '
                                                                  'backprop')
parser.add_argument('--model_address', type=str, help='model address')
...
def name_date(model_file):    
    parts = model_file.split('-')
    modify_coefficient = parts[3]
    modify_regularization = parts[4]
    batch_size = parts[5]
...
for i in range(args.n_runs):
  ...
  # model_path=f'./saved_models/{args.prefix}-{args.modify_coefficient}-{args.modify_regularization}-{args.lr}-{args.bs}-{args.data}.pth'
  ...
  for epoch in range(args.n_epoch):
    ...
    for k in range(num_batch):
      ...
      with torch.no_grad():
        source_embedding, destination_embedding, _, regularization_norm = tgn.compute_temporal_embeddings(sources_batch,
                                                                                    destinations_batch,
                                                                                    destinations_batch,
                                                                                    timestamps_batch,
                                                                                    edge_idxs_batch,
      ...
      # + args.modify_regularization*regularization_norm
      decoder_loss.backward()
      decoder_optimizer.step()
      loss += decoder_loss.item()
      regularization_norm_loss += regularization_norm.item()

    train_losses.append(loss / num_batch)
    # train_losses.append(loss)
    # norm_list.append(regularization_norm_loss)
    
    val_ap, val_auc, accuracy = eval_node_classification(tgn.eval(), decoder, val_data, full_data.edge_idxs, BATCH_SIZE,
    ...
    logger.info(f'Epoch {epoch}: train loss: {loss / num_batch}, regularization_loss: {regularization_norm_loss / num_batch}, val auc: {val_auc}, val accuracy: {accuracy}, time: {time.time() - start_epoch}')
    time_log.append(time.time() - start_epoch)
  ...
  if args.use_validation:
    ...
    test_ap, test_auc, test_accuracy = eval_node_classification(tgn, decoder, test_data, full_data.edge_idxs, BATCH_SIZE,
                                        n_neighbors=NUM_NEIGHBORS)
  else:
  ...
  name_list = name_date(MODEL_ADDRESS)
  print(name_list)
  experiment_data = {
      "dataset": args.data,
      "batch_size_original": name_list[0],
      "batch_size_new": args.bs,
      "modify_coefficient": name_list[1],
      "modify_regularization": name_list[2],
      "val_aucs": val_aucs,
      "test_ap": test_ap,
      "test_auc": test_auc,
      "train_losses": train_losses,
