...
class TemporalAttentionLayer(torch.nn.Module):
  ...
  def forward(self, src_node_features, src_time_features, neighbors_features,
              neighbors_time_features, edge_features, neighbors_padding_mask, flag):
    """
    ...
    modify = math.sqrt(query.shape[2]*math.sqrt(query.shape[0]*key.shape[0]))
    modify = modify*self.coefficient
    ...
    # m = 
    if flag == True:
      Mq = self.multi_head_target.q_proj_weight
      Mv = self.multi_head_target.v_proj_weight
      V = torch.matmul(Mv, key.transpose(-2, -1))
      A1 = torch.matmul(Mq, V.T)  # Mq * V^T
      V = V.to(dtype=torch.float32)
      A1 = A1.to(dtype=torch.float32)
      norm_A1 =torch.sqrt(torch.norm(A1, p='fro')/V.shape[1]) 
      norm_A2 = torch.sqrt(torch.norm(V, p='fro')/V.shape[1])
      result = norm_A1 * norm_A2
      return attn_output, attn_output_weights, result
    # return attn_output, attn_output_weights, query, key
